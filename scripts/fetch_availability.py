...
=====================
Pulls Q4 (October-December) refrigerated-truck availability data from the
...
Output: data/q4_availability.json — mean Q4 availability by region &
...
def fetch_all(start_year: int, end_year: int) -> list[dict[str, Any]]:
    ...
    offset = 0
    where = (
        f"date_extract_m(date) IN (10, 11, 12) AND "
...
def aggregate(records: list[dict[str, Any]], end_year: int) -> dict[str, Any]:
    ...
    for rec in records:
        ...
        if d is None or d.month not in Q4_MONTHS:
            continue
    ...
    print(f"  kept {kept:,} Q4 records ({skipped_bad_avail:,} dropped for bad availability) "
    ...
    # ("Oct 6", "Oct 13", …) rather than smearing across the 4-year window.
    #
    # Q4 ISO weeks run ~40-53, but the final days of December can roll into
    # ISO week 1 of the following year. Sort those low weeks *after* the high
    # ones so the x-axis stays in calendar order (Oct → Dec) instead of
    # yanking a late-December point to the far left.
    def _q4_week_sort_key(w: int) -> int:
        return w + 53 if w < 26 else w
    weeks_sorted = sorted(weeks_seen, key=_q4_week_sort_key)
    ...
    # "All Commodities" rollup — every Q4 record, regardless of commodity.
...
def main() -> None:
    ...
    print("Aggregating Q4 (Oct–Dec) by commodity × region × week …")
    ...
    out = {
        "metadata": {
            ...
            "n_years_avg": N_YEARS,
            "metric": "Refrigerated truck availability, weekly mean across Q4 (1=Surplus, 5=Shortage)",
    ...
    out_path = Path(__file__).resolve().parent.parent / "data" / "q4_availability.json"
