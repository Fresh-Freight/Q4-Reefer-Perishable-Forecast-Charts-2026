...
fetch_data.py
=============
Pulls Q4 (October-December) refrigerated truck volume data from the USDA AMS
...
Output: data/q4_volumes.json — aggregated Q4 volumes by region & commodity,
...
def fetch_all(start_year: int, end_year: int) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if detected_date_field:
            # Server-side filter: only Q4 months (Oct/Nov/Dec) within the target
            ...
            params["$where"] = (
                f"date_extract_m({detected_date_field}) IN (10, 11, 12) AND "
...
def aggregate(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    print(f"  kept {kept:,} Q4 records → {len(acc)} commodities × "
    ...
    for commodity, regions in acc.items():
        ...
        for region, by_year in regions.items():
            ...
            if n == 0:
                continue
            month_totals = {10: 0.0, 11: 0.0, 12: 0.0}
            for year_data in by_year.values():
                for m in Q4_MONTHS:
                    month_totals[m] += year_data.get(m, 0.0)
            production[commodity][region] = {
                "oct": round(month_totals[10] / n / 1_000_000, 2),
                "nov": round(month_totals[11] / n / 1_000_000, 2),
                "dec": round(month_totals[12] / n / 1_000_000, 2),
            }

    # Keep the top 75 commodities by Q4 total volume. Past rank ~75 the tail is
    # mostly sub-15M-lb items that ship from a single region and barely move the
    # map.
    commodity_totals = [
    ...
    all_c: dict[str, dict[str, float]] = defaultdict(lambda: {"oct": 0.0, "nov": 0.0, "dec": 0.0})
...
def main() -> None:
    ...
    print("Aggregating Q4 (Oct–Dec) by commodity × region …")
    ...
    out = {
        "metadata": {
            ...
            "n_years_avg": N_YEARS,
            "metric": "Refrigerated truck volume, Q4 (Oct-Dec) average, in millions of pounds",
    ...
    out_path = Path(__file__).resolve().parent.parent / "data" / "q4_volumes.json"
