...
# ---------------------------------------------------------------------------
# Toggle: "api" hits the Socrata endpoint, anything else uses a local CSV.
# Defaults to "api" because that is what CI and the published chart both want;
# pass DATA_SOURCE=csv for an offline local run against a downloaded export.
# ---------------------------------------------------------------------------
DATA_SOURCE = os.environ.get("DATA_SOURCE", "api").strip().lower()
...
# Optional local CSV export, only used when DATA_SOURCE=csv. Resolved against
# this chart's own directory rather than the caller's working directory, since
# CI invokes the script from the repo root. The export is not committed: it is
# ~32 MB, and CI always uses the API path.
CSV_FALLBACK_PATH = os.environ.get(
    "CSV_FALLBACK_PATH",
    str(Path(__file__).resolve().parent.parent
        / "data" / "refrigerated_truck_rates_and_availability.csv"),
)
